...
const App8 = ({navigation:{goBack}}) => {
  ...
  async function onClick() {
    const check = await DataStore.save(
      new Couplings({
        couplingssize: bitssize,
        couplingsused: bitsused,
        couplingsdamage: bitsdamage,
        couplingslost: bitslost,
      }),
    );

    console.log(check);

    Alert.alert('couplings consumbles items added...');
